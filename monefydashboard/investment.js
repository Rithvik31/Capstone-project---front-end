...
function setupInvestment() {
  ...
  apiFetchAllInvestment(table)
}

setupInvestment()
let j = 1;
function propulateInvestmentData(table, invoices) {
...
function apiFetchAllInvestment(table) {
  axios.get('http://localhost:8080/finance/user-finances')
    .then(res => {
      ...
      propulateInvestmentData(table, bd)
    })
...
function showConfirmDeleteModal(id) {
  ...
  btDl.onclick = () => {
    apiCallDeleteInvestment(id, modal)
  }
}


function apiCallDeleteInvestment(id, modal) {
